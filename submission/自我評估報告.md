...
由測試注入腳本 `testdata/create_calendar_events.py`（對應 PDF 的 `createcalendarevents.py`）
透過 Google Calendar 自動建立下列 10 筆事件,agent 查詢時確實回傳全部 10 筆,確認注入成功：
...
由測試注入腳本 `testdata/send_test_emails_smtp.py`（對應 PDF 的 `sendtestemails.py`,
從外部 Gmail 帳號經 SMTP 寄入 agent 信箱）寄出 8 封測試信到受監控信箱,符合規格的 4 類各 2 封：
