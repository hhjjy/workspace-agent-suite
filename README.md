...
> 替代版本:`testdata/send_test_emails_smtp.py` 走 SMTP + 兩個 Gmail 帳號(對齊規範
> sendtestemails.py 語意,模擬外部客戶寄信)。需在 `.env` 額外設 `SMTP_SENDER_EMAIL` /
> `SMTP_APP_PASSWORD` / `SMTP_TARGET_EMAIL`,寄件帳號須開兩階段驗證 + App Password。
...
├── testdata/            # 測試資料注入(對齊 PDF)
│   ├── create_calendar_events.py  # 塞 10 筆事件(PDF Table 1)
│   ├── send_test_emails.py         # 寄 8 封客服信 — MCP/OAuth 自寄自(預設)
│   └── send_test_emails_smtp.py    # 寄 8 封客服信 — SMTP/App Password 兩帳號(可選)
├── demo/                # 離線展示工具
